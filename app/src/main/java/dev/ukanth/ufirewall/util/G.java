...
public class G extends Application implements Application.ActivityLifecycleCallbacks{
   ...
   /* public static boolean showQuickButton() {
    ...
    private static void seedCustomThemeColors(SharedPreferences.Editor editor, String theme) {
        // Custom color controls should start from the selected preset so users
        // can make small theme adjustments without rebuilding every color.
    ...
    private static String normalizeTheme(String val) {
    ...
    public static int getSelectedThemeStyle(Context context) {
        // Custom colors must keep the selected preset as the base style.
        // Switching to a separate custom style caused Android attrs to diverge
        // before the saved palette could be applied.
        switch (getEffectiveSelectedTheme(context)) {
            case "L":
                return R.style.AppLightTheme;
            case "LHC":
                return R.style.AppLightHighContrastTheme;
            case "B":
                return R.style.AppBlackTheme;
            case "A":
                return R.style.AppAmberTheme;
            case "O":
                return R.style.AppOceanTheme;
            case "F":
                return R.style.AppForestTheme;
            case "S":
                return R.style.AppSlateTheme;
            case "P":
                return R.style.AppPlumTheme;
            case "D":
            default:
  ...
  /*  public static boolean isXposedDM() {
   ...
   /* public static boolean blockIPv6() {
    ...
    public static int sysColor(Context context) {
        ...
        // The system-app picker stores a preference, so theme code must only provide
        // defaults. Earlier theme branches could override the saved picker value.
    ...
    private static int defaultSystemAppColor(String theme) {
        switch (normalizeTheme(theme)) {
            ...
            case "B":
                return Color.parseColor("#FDDF6C");
            case "A":
    ...
    private static int defaultBackgroundColor(String theme) {
        switch (normalizeTheme(theme)) {
            ...
            case "B":
                return Color.parseColor("#000000");
            case "A":
                return Color.parseColor("#2B2415");
            case "O":
                return Color.parseColor("#102A43");
            case "F":
                return Color.parseColor("#17251B");
            case "S":
                return Color.parseColor("#1F2428");
            case "P":
                return Color.parseColor("#241B2F");
            case "D":
            default:
    ...
    private static boolean isLightTheme(String theme) {
        return "L".equals(normalizeTheme(theme)) || "LHC".equals(normalizeTheme(theme));
    }
