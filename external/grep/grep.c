...
#endif
{
	grep_list_data_t *gl = malloc(sizeof(*gl));
    if(!gl) {
        puts("out of memory");
        exit(1);
    }
...
int main(int argc UNUSED_PARAM, char **argv)
{
	...
	if (ENABLE_FEATURE_GREP_FGREP_ALIAS && *argv[0] == 'f')
		option_mask32 |= OPT_F;
